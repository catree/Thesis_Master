...
path_images='/home/strub/Image_objects_ICPR_rotationEdited/';
...
[image_size]=size(imageee{1});
image_area=[20 15];
factor_of_height=1.5;
image_area=image_area*factor_of_height;

pose_multiple=[image_size(2),image_size(1)]./image_area;
...
for ii=0:length(xy)-1
   ...
   vis_1(xy(ii+1,1):h(ii+1)+xy(ii+1,1)-1,xy(ii+1,2):w(ii+1)+xy(ii+1,2)-1,1)=rgb2gray(imageee{ii+1});
   
   vis2=(vis(:,:,1));
   figure(44),subplot(121),imshow(vis_1,[]),subplot(122),
   imshow(vis_1(xy(ii+1,1):h(ii+1)+xy(ii+1,1)-1, xy(ii+1,2):w(ii+1)+xy(ii+1,2)-1,1),[]);
   
   vis_3(xy(ii+1,1):h(ii+1)+xy(ii+1,1)-1, xy(ii+1,2):w(ii+1)+xy(ii+1,2)-1,1)=(vis_3(xy(ii+1,1):h(ii+1)+xy(ii+1,1)-1, xy(ii+1,2):w(ii+1)+xy(ii+1,2)-1,1))+double(rgb2gray(imageee{ii+1}));
   
%    figure(44),subplot(121),imshow(vis_1,[]),subplot(122),
%    imshow(vis_1(xy(ii+1,1):h(ii+1)+xy(ii+1,1)-1, xy(ii+1,2):w(ii+1)+xy(ii+1,2)-1,1),[]);
%    
   
   vis_5R(xy(ii+1,1):h(ii+1)+xy(ii+1,1)-1,xy(ii+1,2):w(ii+1)+xy(ii+1,2)-1)=imageee{ii+1}(:,:,1);
   vis_5G(xy(ii+1,1):h(ii+1)+xy(ii+1,1)-1,xy(ii+1,2):w(ii+1)+xy(ii+1,2)-1)=imageee{ii+1}(:,:,2);
   vis_5B(xy(ii+1,1):h(ii+1)+xy(ii+1,1)-1,xy(ii+1,2):w(ii+1)+xy(ii+1,2)-1)=imageee{ii+1}(:,:,3);
