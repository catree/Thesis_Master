...
#define Dwindow	10
 ...
 public:
	...
	Bug2Vrep::Bug2Vrep()
	{
		...
		GoalPose_publisher = nh_.advertise<geometry_msgs::Point>("/vrep/GoalPose_sub",1);
...
/*
  ...
  */
    ...
    float theta = atan2((TargetPosition.y - bvrep.QuadPos.y),(TargetPosition.x - bvrep.QuadPos.x));
	Delta.x = 0.25*(cos(theta));
	Delta.y = 0.25*(sin(theta));
