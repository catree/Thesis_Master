...
#define MOVE_STEP 0.5 // 0.07 originally 
	...
	while(ros::ok())
	{
		...
		if(!bvrep.mot_2_goal)
		{
			...
			{	
				...
				if(temp_angle<0)
					temp_angle = temp_angle + 360;
				TargetPosition.x+=value*cos(bvrep.Deg2Rad(temp_angle));
				TargetPosition.y+=value*sin(bvrep.Deg2Rad(temp_angle));	
			} 
			else
			{
